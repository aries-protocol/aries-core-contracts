...
contract Borrowable is IBorrowable, PoolToken, BStorage, BSetter, BInterestRateModel, BAllowance {
    ...
    function _mintReserves(uint256 _exchangeRate, uint256 _totalSupply) internal returns (uint256) {
        ...
        if (_exchangeRate > _exchangeRateLast) {
            uint256 _exchangeRateNew = _exchangeRate.sub(_exchangeRate.sub(_exchangeRateLast).mul(reserveFactor).div(1e18));
    ...
    function borrow(
        ...
        require(borrowAmount <= _totalBalance, "Aries: INSUFFICIENT_CASH");
        _checkBorrowAllowance(borrower, msg.sender, borrowAmount);

        // optimistically transfer funds
        if (borrowAmount > 0) _safeTransfer(receiver, borrowAmount);
        if (data.length > 0) IAriesCallee(receiver).ariesBorrow(msg.sender, borrower, borrowAmount, data);
        ...
        if (adjustedBorrowAmount > repayAmount)
            require(ICollateral(collateral).canBorrow(borrower, address(this), accountBorrows), "Aries: INSUFFICIENT_LIQUIDITY");
