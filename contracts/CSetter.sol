...
contract CSetter is PoolToken, CStorage {
    ...
    function _initialize(
        ...
        require(msg.sender == factory, "Aries: UNAUTHORIZED"); // sufficient check
        _setName(_name, _symbol);
        underlying = _underlying;
        borrowable0 = _borrowable0;
        borrowable1 = _borrowable1;
        ariesPriceOracle = IFactory(factory).ariesPriceOracle();
    }
    ...
    function _checkSetting(
        ...
        _checkAdmin();
        require(parameter >= min, "Aries: INVALID_SETTING");
        require(parameter <= max, "Aries: INVALID_SETTING");
    }

    function _checkAdmin() internal view {
        require(msg.sender == IFactory(factory).admin(), "Aries: UNAUTHORIZED");
    }
}
