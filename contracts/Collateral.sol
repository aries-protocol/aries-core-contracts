...
contract Collateral is ICollateral, PoolToken, CStorage, CSetter {
    ...
    function getPrices() public returns (uint256 price0, uint256 price1) {
        (uint224 twapPrice112x112, ) = IAriesPriceOracle(ariesPriceOracle).getResult(underlying);
         ...
         */
        require(price0 > 100, "Aries: PRICE_CALCULATION_ERROR");
        require(price1 > 100, "Aries: PRICE_CALCULATION_ERROR");
    }
    ...
    function _transfer(
        ...
        require(tokensUnlocked(from, value), "Aries: INSUFFICIENT_LIQUIDITY");
    ...
    function canBorrow(
        ...
        require(borrowable == _borrowable0 || borrowable == _borrowable1, "Aries: INVALID_BORROWABLE");
    ...
    function seize(
        ...
        require(msg.sender == borrowable0 || msg.sender == borrowable1, "Aries: UNAUTHORIZED");

        (, uint256 shortfall) = accountLiquidity(borrower);
        require(shortfall > 0, "Aries: INSUFFICIENT_SHORTFALL");
        ...
        balanceOf[borrower] = balanceOf[borrower].sub(seizeTokens, "Aries: LIQUIDATING_TOO_MUCH");
    ...
    function flashRedeem(
        ...
        if (data.length > 0) IAriesCallee(redeemer).ariesRedeem(msg.sender, redeemAmount, data);
        ...
        require(redeemTokens >= declaredRedeemTokens, "Aries: INSUFFICIENT_REDEEM_TOKENS");
