const {
    ...
    makeUniswapV2Pair,
    makeAriesPriceOracle,
    makeBDeployer,
    makeCDeployer,
    makeFactory,
} = require("./Utils/Aries");
const { expectEqual, expectAlmostEqualMantissa, expectRevert, expectEvent, bnMantissa } = require("./Utils/JS");
const { address, encodePacked } = require("./Utils/Ethereum");
...
function getCollateralAddress(deployerAddress, factoryAddress, uniswapV2PairAddress) {
    ...
    return getCreate2Address(["0xff", deployerAddress, salt, keccak256(CollateralProduction.bytecode)]);
}
...
function getBorrowableAddress(deployerAddress, factoryAddress, uniswapV2PairAddress, index) {
    ...
    return getCreate2Address(["0xff", deployerAddress, salt, keccak256(BorrowableProduction.bytecode)]);
}
...
contract("Factory", function (accounts) {
    ...
    describe("constructor", () => {
        it("correctly initialize Factory", async () => {
            ...
            const ariesPriceOracle = address(4);
            const factory = await Factory.new(admin, reservesAdmin, bDeployer, cDeployer, ariesPriceOracle);
            ...
            expect(await factory.ariesPriceOracle()).to.eq(ariesPriceOracle);
        });
    });
    ...
    describe("create lending pool", () => {
        ...
        before(async () => {
            ...
            ca = factory.obj.cDeployer.address;
            ba = factory.obj.bDeployer.address;
            fa = factory.address;
        ...
        it("collateral and borrowable addresses are dependent on factory", () => {
            expect(getCollateralAddress(ca, fa, uniswapV2Pair1.address)).to.not.eq(getCollateralAddress(ca, root, uniswapV2Pair1.address));
        ...
        it("revert if already exists", async () => {
            await expectRevert(factory.createCollateral(uniswapV2Pair1.address), "Aries: ALREADY_EXISTS");
            await expectRevert(factory.createBorrowable0(uniswapV2Pair2.address), "Aries: ALREADY_EXISTS");
            await expectRevert(factory.createBorrowable1(uniswapV2Pair3.address), "Aries: ALREADY_EXISTS");
        });
        ...
        it("initialize revert if not all three contracts are deployed", async () => {
            await expectRevert(factory.initializeLendingPool(uniswapV2Pair1.address), "Aries: BORROWABLE1_NOT_CREATED");
            await expectRevert(factory.initializeLendingPool(uniswapV2Pair2.address), "Aries: COLLATERALIZABLE_NOT_CREATED");
            await expectRevert(factory.initializeLendingPool(uniswapV2Pair3.address), "Aries: BORROWABLE0_NOT_CREATED");
        });
        ...
        it("only the factory can initialize PoolTokens", async () => {
            ...
            await expectRevert(
                (await Collateral.at(lendingPool.collateral))._initialize("", "", address(0), address(0), address(0)),
                "Aries: UNAUTHORIZED"
            );
            await expectRevert(
                (await Borrowable.at(lendingPool.borrowable0))._initialize("", "", address(0), address(0)),
                "Aries: UNAUTHORIZED"
            );
            await expectRevert(
                (await Borrowable.at(lendingPool.borrowable1))._initialize("", "", address(0), address(0)),
                "Aries: UNAUTHORIZED"
            );
        });
        it("factory can only be set once", async () => {
            const lendingPool = await factory.getLendingPool(uniswapV2Pair1.address);
            await expectRevert((await Collateral.at(lendingPool.collateral))._setFactory(), "Aries: FACTORY_ALREADY_SET");
            await expectRevert((await Borrowable.at(lendingPool.borrowable0))._setFactory(), "Aries: FACTORY_ALREADY_SET");
            await expectRevert((await Borrowable.at(lendingPool.borrowable1))._setFactory(), "Aries: FACTORY_ALREADY_SET");
        });
        ...
        it("ariesPriceOracle can be initialized or not", async () => {
            expect((await factory.obj.ariesPriceOracle.getPair(uniswapV2Pair1.address)).initialized).to.eq(false);
            expect((await factory.obj.ariesPriceOracle.getPair(uniswapV2Pair2.address)).initialized).to.eq(false);
            await factory.obj.ariesPriceOracle.initialize(uniswapV2Pair3.address);
            expect((await factory.obj.ariesPriceOracle.getPair(uniswapV2Pair3.address)).initialized).to.eq(true);
        });
        it("initialize", async () => {
        ...
        it("ariesPriceOracle is initialized correctly", async () => {
            expect((await factory.obj.ariesPriceOracle.getPair(uniswapV2Pair1.address)).initialized).to.eq(true);
            expect((await factory.obj.ariesPriceOracle.getPair(uniswapV2Pair2.address)).initialized).to.eq(true);
            expect((await factory.obj.ariesPriceOracle.getPair(uniswapV2Pair3.address)).initialized).to.eq(true);
        });
        it("revert if already initialized", async () => {
            await expectRevert(factory.initializeLendingPool(uniswapV2Pair1.address), "Aries: ALREADY_INITIALIZED");
            await expectRevert(factory.initializeLendingPool(uniswapV2Pair2.address), "Aries: ALREADY_INITIALIZED");
            await expectRevert(factory.initializeLendingPool(uniswapV2Pair3.address), "Aries: ALREADY_INITIALIZED");
        });
    });
    ...
    describe("admin", () => {
        ...
        it("change admin", async () => {
            await expectRevert(factory._setPendingAdmin(root, { from: root }), "Aries: UNAUTHORIZED");
            await expectRevert(factory._setPendingAdmin(root, { from: reservesAdmin }), "Aries: UNAUTHORIZED");
            await expectRevert(factory._acceptAdmin({ from: root }), "Aries: UNAUTHORIZED");
        ...
        it("change reserves admin", async () => {
            await expectRevert(factory._setReservesPendingAdmin(root, { from: root }), "Aries: UNAUTHORIZED");
            await expectRevert(factory._setReservesPendingAdmin(root, { from: admin }), "Aries: UNAUTHORIZED");
            await expectRevert(factory._acceptReservesAdmin({ from: root }), "Aries: UNAUTHORIZED");
        ...
        it("change reserves manager", async () => {
            await expectRevert(factory._setReservesManager(reservesManager, { from: reservesManager }), "Aries: UNAUTHORIZED");
            await expectRevert(factory._setReservesManager(reservesManager, { from: admin }), "Aries: UNAUTHORIZED");
