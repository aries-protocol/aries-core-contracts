const { makeFactory, makeUniswapV2Pair } = require("./Utils/Aries");
const { expectEqual, expectAlmostEqualMantissa, expectRevert, expectEvent, bnMantissa, BN, uq112 } = require("./Utils/JS");
const { freezeTime, increaseTime } = require("./Utils/Ethereum");
...
contract("Highlevel", function (accounts) {
    ...
    const expectedAccountLiquidityA = bnMantissa(69.556);
    ...
    it("deploy lending pool", async () => {
        ...
        await factory.obj.ariesPriceOracle.setPrice(uniswapV2Pair.address, uq112(price0A / price1A));
    ...
    it("borrow token1 fails", async () => {
        await expectRevert(borrowable1.borrow(borrower, borrower, lendAmount1, "0x", { from: borrower }), "Aries: INSUFFICIENT_LIQUIDITY");
    });
    ...
    it("liquidation fail", async () => {
        await factory.obj.ariesPriceOracle.setPrice(uniswapV2Pair.address, uq112(price0B / price1B));
        ...
        await expectRevert(borrowable0.liquidate(borrower, liquidator), "Aries: INSUFFICIENT_SHORTFALL");
    });

    it("liquidate token0", async () => {
        await factory.obj.ariesPriceOracle.setPrice(uniswapV2Pair.address, uq112(price0C / price1C));
        const { liquidity, shortfall } = await collateral.accountLiquidity.call(borrower);
