...
const { encodeParameters, etherBalance, etherUnsigned, address, encode, encodePacked } = require("./Ethereum");
...
const AriesERC20 = artifacts.require("AriesERC20Harness");
...
const AriesCallee = artifacts.require("AriesCallee");
...
async function makeErc20Token(opts = {}) {
    ...
    return await AriesERC20.new(name, symbol);
}
...
async function makeAriesPriceOracle(opts = {}) {
...
async function makeFactory(opts = {}) {
    ...
    const uniswapV2Factory = opts.uniswapV2Factory || (await makeUniswapV2Factory(opts));
    const ariesPriceOracle = opts.ariesPriceOracle || (await makeAriesPriceOracle(opts));
    const factory = await Factory.new(admin, reservesAdmin, bDeployer.address, cDeployer.address, ariesPriceOracle.address);
...
async function sendPermit(opts) {
    ...
    const digest = await getApprovalDigest(name, token.address, { owner, spender, value }, nonce, deadline);
...
async function sendBorrowPermit(opts) {
    ...
    const name = await token.name();
    const nonce = await token.nonces(owner);
    const digest = await getBorrowApprovalDigest(name, token.address, { owner, spender, value }, nonce, deadline);
    const { v, r, s } = ecsign(Buffer.from(digest.slice(2), "hex"), Buffer.from(private_key, "hex"));
