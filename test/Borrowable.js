...
const { expectAlmostEqualMantissa, expectRevert, expectEvent, bnMantissa, BN } = require("./Utils/JS");
...
contract("Borrowable", function (accounts) {
    ...
    describe("borrow and repay", () => {
        ...
        async function makeBorrow(params) {
            const { borrowAmount, repayAmount, maxBorrowableNew } = params;
            ...
            if (maxBorrowable.lt(expectedAccountBorrows)) {
                await expectRevert(borrowAction, "Aries: INSUFFICIENT_LIQUIDITY");
                return false;
            }
        ...
        before(async () => {
            ...
            receiver = (await AriesCallee.new(recipient.address, underlying.address)).address;
        ...
        it(`fail if cash is insufficient`, async () => {
            ...
            await borrowable.sync();
            await expectRevert(borrowable.borrow(borrower, receiver, "1", "0x"), "Aries: INSUFFICIENT_CASH");
        });

        it(`fail if not allowed`, async () => {
            await underlying.setBalanceHarness(borrowable.address, "1");
            await borrowable.sync();
            await expectRevert(borrowable.borrow(borrower, receiver, "1", "0x"), "Aries: BORROW_NOT_ALLOWED");
        });
    ...
    describe("liquidate", () => {
        ...
        it(`fail if shortfall is insufficient`, async () => {
            ...
            await expectRevert(borrowable.liquidate(borrower, liquidator), "Aries: INSUFFICIENT_SHORTFALL");
        });
        ...
        it(`fail if there is not enough collateral`, async () => {
            ...
            await expectRevert(borrowable.liquidate(borrower, liquidator), "Aries: LIQUIDATING_TOO_MUCH");
        });
    ...
    describe("reentrancy", () => {
        ...
        it(`borrow reentrancy`, async () => {
            await expectRevert(borrowable.borrow(address(0), receiver, "0", encode(["uint"], [1])), "Aries: REENTERED");
            await expectRevert(borrowable.borrow(address(0), receiver, "0", encode(["uint"], [2])), "Aries: REENTERED");
            await expectRevert(borrowable.borrow(address(0), receiver, "0", encode(["uint"], [3])), "Aries: REENTERED");
            await expectRevert(borrowable.borrow(address(0), receiver, "0", encode(["uint"], [4])), "Aries: REENTERED");
            await expectRevert(borrowable.borrow(address(0), receiver, "0", encode(["uint"], [5])), "Aries: REENTERED");
            await expectRevert(borrowable.borrow(address(0), receiver, "0", encode(["uint"], [6])), "Aries: REENTERED");
