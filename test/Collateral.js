const { Borrowable, Collateral, AriesCallee, ReentrantCallee, Recipient, makeFactory, makeUniswapV2Pair } = require("./Utils/Aries");
const { expectAlmostEqualMantissa, expectRevert, expectEvent, expectEqual, bnMantissa, uq112, BN } = require("./Utils/JS");
const { address, encode } = require("./Utils/Ethereum");
...
contract("Collateral", function (accounts) {
    ...
    describe("getPrices", () => {
        ...
        before(async () => {
            ...
            await collateral.setPriceOracle(factory.obj.ariesPriceOracle.address);
        ...
        [
            ...
            it(`getPrices for ${JSON.stringify(testCase)}`, async () => {
                ...
                await factory.obj.ariesPriceOracle.setPrice(underlying.address, uq112(priceOracle));
        ...
        it(`fail if price(0|1) <= 100`, async () => {
            ...
            await factory.obj.ariesPriceOracle.setPrice(underlying.address, priceUQ112);
            await underlying.setTotalSupply(totalSupply);
            await underlying.setReserves(reserve0, reserve1);
            await expectRevert(collateral.getPrices(), "Aries: PRICE_CALCULATION_ERROR");
    ...
    [
        { safetyMargin: 2.5, liquidationIncentive: 1.01, amounts: [280, 100, 100], prices: [1, 1] },
        { safetyMargin: 2.25, liquidationIncentive: 1.02, amounts: [3060, 0, 2000], prices: [1, 1] },
        { safetyMargin: 2.0, liquidationIncentive: 1.03, amounts: [1000, 111, 1.546], prices: [11.3, 0.56] },
        { safetyMargin: 1.75, liquidationIncentive: 1.04, amounts: [11.3, 175.6, 200], prices: [0.0059, 0.034] },
        { safetyMargin: 1.5, liquidationIncentive: 1.05, amounts: [2154546, 1, 1.12e12], prices: [1154546, 0.0000008661] },
    ].forEach((testCase) => {
        describe(`Collateral tests for ${JSON.stringify(testCase)}`, () => {
            ...
            it(`calculateLiquidity`, async () => {
                const { liquidity, shortfall } = await collateral.calculateLiquidity.call(
                    bnMantissa(amounts[0]),
                    bnMantissa(amounts[1]),
                    bnMantissa(amounts[2])
                );
            ...
            it(`transfer:fail`, async () => {
                ...
                await expectRevert(collateral.transfer(address(0), transferAmount, { from: user }), "Aries: INSUFFICIENT_LIQUIDITY");
            });
            ...
            it(`accountLiquidityAmounts`, async () => {
                const { liquidity, shortfall } = await collateral.accountLiquidityAmounts.call(
                    user,
                    bnMantissa(amounts[1]),
                    bnMantissa(amounts[2])
                );
    ...
    describe("seize", () => {
        ...
        it(`fail if msg.sender is not borrowable`, async () => {
            await expectRevert(collateral.seize(address(0), address(0), "0"), "Aries: UNAUTHORIZED");
        });

        it(`fail if shortfall is insufficient`, async () => {
            await collateral.setAccountLiquidityHarness(borrower, "0", "0");
            await expectRevert(borrowable0.seizeHarness(collateral.address, liquidator, borrower, "0"), "Aries: INSUFFICIENT_SHORTFALL");
            await expectRevert(borrowable1.seizeHarness(collateral.address, liquidator, borrower, "0"), "Aries: INSUFFICIENT_SHORTFALL");
        });
    ...
    describe("flash redeem", () => {
        ...
        before(async () => {
            ...
            callee = (await AriesCallee.new(recipient.address, collateral.address)).address;
        });
        ...
        it("redeem fails if redeemTokens is not enough", async () => {
            ...
            await expectRevert(collateral.flashRedeem(user, redeemAmount, "0x"), "Aries: INSUFFICIENT_REDEEM_TOKENS");
        });
        ...
        it("redeem fails if redeemAmount exceeds cash", async () => {
            await expectRevert(collateral.flashRedeem(user, collateralBalancePrior.add(new BN(1)), "0x"), "Aries: INSUFFICIENT_CASH");
        });
    ...
    describe("reentrancy", () => {
        ...
        it(`borrow reentrancy`, async () => {
            await expectRevert(collateral.flashRedeem(receiver, "0", encode(["uint"], [1])), "Aries: REENTERED");
            await expectRevert(collateral.flashRedeem(receiver, "0", encode(["uint"], [2])), "Aries: REENTERED");
            await expectRevert(collateral.flashRedeem(receiver, "0", encode(["uint"], [3])), "Aries: REENTERED");
            await expectRevert(collateral.flashRedeem(receiver, "0", encode(["uint"], [4])), "Aries: REENTERED");
            await expectRevert(collateral.flashRedeem(receiver, "0", encode(["uint"], [5])), "Aries: REENTERED");
