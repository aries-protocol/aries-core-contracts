const { AriesERC20, getDomainSeparator, getApprovalDigest, sendPermit } = require("./Utils/Aries");
...
contract("AriesERC20", function (accounts) {
    let root = accounts[0];
    let user = accounts[1];
    ...
    beforeEach(async () => {
        token = await AriesERC20.new(NAME, SYMBOL);
    ...
    it("transfer:fail", async () => {
        await expectRevert(token.transfer(other, TOTAL_SUPPLY.add(new BN(1)), { from: user }), "Aries: TRANSFER_TOO_HIGH");
        await expectRevert(token.transfer(user, "1", { from: other }), "Aries: TRANSFER_TOO_HIGH");
    });
    ...
    it("transferFrom:fail", async () => {
        await expectRevert(token.transferFrom(user, other, TEST_AMOUNT, { from: other }), "Aries: TRANSFER_NOT_ALLOWED");
    });
